...
void UJSBSimMovementComponent::LoadAircraft(bool ResetToDefaultSettings)
{
  ...
  DeInitializeJSBSim();
  InitializeJSBSim();
  AircraftLoaded = Exec->LoadModel(TCHAR_TO_UTF8(*AircraftModel));
...
void UJSBSimMovementComponent::InitializeJSBSim()
{
  if (!JSBSimInitialized)
  {
    ...
    // Set data paths...
    FString AircraftPath = FPaths::Combine(*RootDir, TEXT("aircraft"));
    FString EnginePath = FPaths::Combine(*RootDir, TEXT("engine"));
    FString SystemPath = FPaths::Combine(*RootDir, TEXT("systems"));

    Exec->SetRootDir(TCHAR_TO_UTF8(*RootDir));
    Exec->SetAircraftPath(TCHAR_TO_UTF8(*AircraftPath));
    Exec->SetEnginePath(TCHAR_TO_UTF8(*EnginePath));
    Exec->SetSystemsPath(TCHAR_TO_UTF8(*SystemPath));
...
void UJSBSimMovementComponent::PrepareModelForCompVisualizer()
{
  ...
  {
    if (Exec == nullptr || !AircraftModel.Compare(FString(Exec->GetModelNameCString()), ESearchCase::IgnoreCase))
    {
      LoadAircraft(false);
      PrepareJSBSim();
