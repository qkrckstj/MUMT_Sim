...
public:
  ...
  /** C-string API to avoid std::string ABI issues across DLL boundaries. */
  bool LoadModel(const char* model, bool addModelToPath = true);
  ...
  bool SetEnginePath(const char* path);
  ...
  bool SetAircraftPath(const char* path);
  ...
  bool SetSystemsPath(const char* path);
  ...
  const char* GetModelNameCString(void) const;
